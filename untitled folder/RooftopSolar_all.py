...
with open('masks1.pickle','wb') as handle:
    pickle.dump(tmasks1,handle,protocol = pickle.HIGHEST_PROTOCOL)
print('COMPLETED MASK1')
'''
img_masks1 = pickle.load( open( "image_masks1", "rb" ) )
...
end_masks = time.time() - start_masks

print(end_masks)
tmasks2_1 = dict(zip(second_one,img_masks2_1))

start_pickle = time.time()
with open('masks2_1.pickle','wb') as handle:
    pickle.dump(tmasks2_1,handle,protocol = pickle.HIGHEST_PROTOCOL)
end_pickle = time.time() - start_pickle
...
#np.savetxt('img_masks2_2.txt',img_masks2_2)
'''
with open('masks2_2.json','w') as handle:
    json.dump(tmasks2_2,handle)
'''
...
#np.savetxt('img_masks2_3.txt',img_masks2_3)
'''
with open('masks2_3.json','w') as handle:
    json.dump(tmasks2_3,handle)
'''
...
'''
with open('masks3.json','w') as handle:
    json.dump(tmasks3,handle)
'''
...
#np.savetxt('img_masks4.txt',img_masks4)


'''
with open('masks4.json','w') as handle:
    json.dump(tmasks4,handle)
'''
print('COMPLETED MASK4')

print('FINISHED MASK GENERATION')
#tmasks = dict(zip(tfname,img_masks))
...
complete_masks = {}
all_m = (tmasks2_2,tmasks2_3,tmasks3,tmasks4)
for d in all_m:
    complete_masks.update(d)
'''
with open('all_masks.json','w') as handle:
    json.dump(complete_masks,handle)
'''
